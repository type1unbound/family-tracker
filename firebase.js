...
window.addEventListener('DOMContentLoaded', function() {
    window.handlePhotoUpload = async function(event) {
        ...
        try {
            ...
            if (uploadArea) {
                uploadArea.innerHTML = '<div style="padding: 40px; text-align: center;">Processing image...</div>';
            }
            ...
            reader.onload = function(e) {
                ...
                img.onload = function() {
                    ...
                    // Set max dimensions (smaller = smaller file size)
                    const MAX_WIDTH = 400;
                    ...
                    // Convert to base64 with compression (0.7 quality = ~70% compression)
                    const compressedBase64 = canvas.toDataURL('image/jpeg', 0.7);
                    
                    console.log('📸 Original size:', e.target.result.length, 'bytes');
                    console.log('📸 Compressed size:', compressedBase64.length, 'bytes');
                    console.log('📸 Compression ratio:', Math.round((1 - compressedBase64.length / e.target.result.length) * 100) + '%');
                    
                    // Check if still too large (Firestore limit is ~1MB)
                    if (compressedBase64.length > 900000) {
                        alert('Image is still too large after compression. Please use a smaller image or lower resolution photo.');
                        if (uploadArea) {
                    ...
                    StateManager.state.tempPhoto = compressedBase64;
                    
                    console.log('✅ Photo compressed and ready to save');
                    
                    // Show preview
                    const photoContainer = document.getElementById('photo-preview-container');
                    if (photoContainer) {
                        photoContainer.innerHTML = `<img src="${compressedBase64}" class="photo-preview" style="max-width: 200px; border-radius: 50%;">`;
                    }
